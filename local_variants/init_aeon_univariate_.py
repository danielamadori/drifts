...
import sys
import os
# Add parent directory to path to import cost_function
...
def connect_redis(port=6379):
    ...
    for name, db_id in db_mapping.items():
        try:
            ...
            raise Exception(f"Failed to connect to Redis DB {db_id} ({name})")
...
def train_and_convert_forest(X_train, y_train, X_test, y_test, rf_params, feature_names,
                               test_split=None, random_state=42):
    """
...
def process_all_classified_samples(connections, dataset_name, class_label, our_forest, 
                                 X_test, y_test, feature_names, eu_data, sigmas):
    """
    ...
    for i, (sample, actual_label) in enumerate(zip(X_test, y_test)):
    ...
    for idx, sample_data in enumerate(target_samples_data):
        ...
        data_entry = {
            ...
            'sigmas' : sigmas[sample_data["test_index"]]
        }
        ...
        try:
            ...
            cost = cost_function(
                ...
                icf=sample_icf, sigmas=sigmas[sample_data['test_index']]
            )
...
def initialize_seed_candidate(connections, sample_dict, our_forest, eu_data, sigmas):
    ...
    print('porca troisa sample dict:', sample_dict)

    cost = cost_function(
        sample=sample_dict['sample_dict'],
        icf=forest_icf, sigmas=sigmas[sample_dict["test_index"]]
    )
    # Store ICF bitmap in R with metadata
    icf_metadata = {
    ...
    if insert_to_pr(connections['PR'], bitmap_string):
...
Examples:
  ...
  python init_aeon_univariate.py ECG200 --class-label "1"
  ...
  # Process GunPoint dataset with specific parameters
  python init_aeon_univariate.py GunPoint --class-label "1" --n-estimators 30 --min-samples-split 3
        """
    )
    ...
    try:
        ...
        stored_samples, summary = process_all_classified_samples(
            ...
            our_forest, X_test, y_test, feature_names, eu_data, sigmas
        )
        ...
        for sample in stored_samples:
            ...
            seed_bitmap, seed_icf = initialize_seed_candidate(
                connections, sample_dict, our_forest, eu_data, sigmas
            )
