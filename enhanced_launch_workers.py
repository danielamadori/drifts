...
class WorkerManager:
    ...
    def load_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if config_path:
            ...
            print(f"[FILE] Loading configuration from {config_file}")
            ...
            for possible in possible_configs:
                if possible.exists():
                    print(f"[FILE] Auto-detected configuration file: {possible}")
                    try:
    ...
    def get_worker_config(self, profile: Optional[str] = None) -> Dict[str, Any]:
        ...
        if profile:
            ...
            print(f"[TARGET] Using profile: {profile}")
            return workers
        else:
    ...
    def start_workers_from_config(self, profile: Optional[str] = None, worker_groups: Optional[List[str]] = None):
        ...
        if worker_groups:
            ...
            if not filtered_configs:
                ...
                print(f"[TIP] Available groups: {list(worker_configs.keys())}")
                return False
            worker_configs = filtered_configs
        
        print(f"[START] Starting workers from configuration")
        print(f"[INFO] Worker groups: {list(worker_configs.keys())}")
        ...
        if profile and profile in self.config.get('profiles', {}):
            ...
            if 'redis' in profile_config:
                ...
                print(f"[INFO] Using Redis from profile '{profile}': {redis_config['host']}:{redis_config['port']}")
            else:
                # Fall back to top-level Redis config
                redis_config = self.config.get('redis', redis_config)
                print(f"[INFO] Using top-level Redis config: {redis_config['host']}:{redis_config['port']}")
        else:
        ...
        for group_name, group_config in worker_configs.items():
            ...
            print(f"\n[ACTION] Starting {count} instances of {script} (group: {group_name})")
            ...
            for i in range(count):
                ...
                print(f"  [ACTION] Starting {worker_id}...")
        ...
        print(f"[FOLDER] Logs directory: {self.logs_dir.absolute()}")
        print(f"[ACTION] Worker PIDs saved to: {self.pids_file}")
    ...
    def start_workers(self, num_workers, worker_script='worker_cache.py', extra_args=None):
        ...
        if not os.path.exists(worker_script):
            ...
            print(f"[TIP] Available workers: {available}")
            return False
        ...
        print(f"[INFO] Using Redis config: {redis_config['host']}:{redis_config['port']}")
    ...
    def stop_workers(self, worker_ids=None):
        ...
        if not workers_to_stop:
            print("[INFO] No matching workers found")
            return True
    ...
    def show_status(self):
        ...
        print(f"\n[INFO] Worker Status ({len(pids_data)} workers)")
        print("=" * 100)
    ...
    def view_logs(self, worker_id=None, tail=False, follow=False, lines=50):
        ...
        if worker_id:
            ...
            print(f"[INFO] Viewing log for {worker_id}: {log_file.name}")
    ...
    def _tail_follow(self, log_file):
        ...
        print(f"[SCAN] Following {log_file.name} (Press Ctrl+C to stop)")
    ...
    def cleanup_old_logs(self, days=None):
        ...
        print(f"[CLEAN] Cleaning up logs older than {days} days...")
    ...
    def clean_restart(self, profile: Optional[str] = None, worker_groups: Optional[List[str]] = None):
        ...
        print("[INFO] Starting clean restart sequence...")
        print("=" * 60)
    ...
    def show_config(self):
        ...
        print("[FILE] Current Configuration:")
        print("=" * 50)
